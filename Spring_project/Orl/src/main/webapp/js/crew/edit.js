...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (!data.auth) {
                ...
                window.location.href = url2 + '/crew/detail?crewIdx=' + data.info.crewIdx + '';
            }

            //hashtag
            if (data.info.crewTag != null) {
                let str = [];
                const crewTag = data.info.crewTag;
                str = crewTag.split(",");
    ...
    $('#submit').on('click', function () {
        ...
        formData.append("crewIdx", crewIdx);
